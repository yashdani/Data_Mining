...
class img_cap:
    def __init__(self):
        ...
        self.meta_cols = {"url": None, "caption": None}
        self.meta_data = pd.read_csv('caption_img.csv', usecols=self.meta_cols.keys())
        ...
        if os.path.isfile("invertedPickle.pkl"):
            self.inverted_index = pickle.load(open('invertedPickle.pkl', 'rb'))
            self.document_vector = pickle.load(open('documentPickle.pkl', 'rb'))
        else:
    ...
    def save(self):
        pickle.dump(self.inverted_index, open('invertedPickle.pkl', 'wb+'))
        pickle.dump(self.document_vector, open('documentPickle.pkl', 'wb+'))
        # print("Saved both")
    ...
    def get_img_cap(self, sorted_score_list, tf_new, idf_new, tf_idf_new):
        result = []
        for entry in sorted_score_list:
            doc_id = entry[0]
            row = self.meta_data.loc[doc_id]
            info = (row["url"],
                    row["caption"] if isinstance(row["caption"], str) else "", entry[1], idf_new[doc_id],
                    tf_new[doc_id], tf_idf_new[doc_id])
            result.append(info)
        new_score = None
        # print(result[0:5])
        return result
